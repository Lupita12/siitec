...
//recupero las opcion tanto de borrado como modificado para ver que opcion entrara en el case
	...
	$un=$_POST['unidadmedida'];
	$val=$_POST['valorcombo'];
	if($val==0)
	{
		mysql_query("insert into descripcion (descripcion, id_articulo, precio) values ('".$de."', ".$ar.",0)");	
		$id_descripcion=mysql_insert_id();
		mysql_query("insert into unidadmedida (nombre, precio, id_descripcion) values ('".$un."', 0,".$id_descripcion.")");	
	}
	else
	{
		mysql_query("insert into unidadmedida (nombre, precio, id_descripcion) values ('".$un."', 0,".$val.")");
		echo "insert into unidadmedida (nombre, precio, id_descripcion) values ('".$un."', 0,".$val.")";
	}
/*
if($opcion == "")
{
...
case 1:
	...
	//list($id_articulo, $y) = split('separa', $x2);
	//list($id_marca, $y1,$y2) = split('separa', $x3);
	//list($id_clasificacion, $y3,$y4) = split('separa', $x5);
	//list($id_umedida, $y5) = split('separa', $x6);
	mysql_query("update descripcion set precio=".$x9." where id_descripcion=".$id_descripcion);
	mysql_query("insert into requisicion_articulo (folio_requisicion, id_articulo,id_partida, cantidad,id_descripcion,precio,total) values (".$_SESSION["idrequi"].", '".$id_articulo."',".$x1.",".$x7.",".$id_descripcion.",".$x9.",".$x.")");
	echo "insert into requisicion_articulo (folio_requisicion, id_articulo,id_partida, cantidad,id_descripcion,precio,total) values (".$_SESSION["idrequi"].", '".$id_articulo."',".$x1.",".$x7.",".$id_descripcion.",".$x9.",".$x.")";
