...
<link href="styles/mios.css" rel="stylesheet" type="text/css" >
<style type="text/css">
<!--
...
<table align="center" border="0" cellpadding="0" cellspacing="0" width="980">
	...
	<tr>
		...
		<table align="center" border="0" cellpadding="0" cellspacing="0" width="99%">
			<tr>
				<td height="80" colspan="2" background="imagenes/head_backend.gif">
					<table width="40%" border="0">
						<tr>
							<td width="26%">&nbsp;</td>
							<td width="74%" class="titulo3bco style1">SIITEC</td>
						</tr>
						<tr>
							<td>&nbsp;</td>
							<td class="titulo2">M&oacute;dulo de Adquisiciones y Presupuesto</td>
						</tr>
				  </table>
				</td>
			</tr>
			<tr>
				<td height="55" colspan="2" valign="middle" bgcolor="#5d5d5d">
					...
					<tr>
						...
						<table border="0" cellpadding="0" cellspacing="0" width="98%">
							<tr>
								...
								<img height="46" src="imagenes/ico_usuario.gif" width="37" ></td>
								<td class="titulo2">Bienvenido</td>
							</tr>
							<tr>
								<td class="txtcontenidobco">Ariel lira Obando</td>
							</tr>
						</table>
						</td>
						<td valign="middle">&nbsp;</td>
						...
						<table border="0" cellpadding="0" cellspacing="0" width="98%">
							...
							<tr>
								<td class="txtcontenidobco">Planeacion </td>
							</tr>
						</table>
						</td>
						<td>&nbsp;</td>
						<td>
						<img height="36" src="imagenes/btn_cerrarsesion.gif" width="130" ></td>
						<td>&nbsp;</td>
					</tr>
				</table>
				</td>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<tr>
						<td colspan="2">
						<img height="5" src="imagenes/blank.gif" width="1" ></td>
					</tr>
					<tr>
						<td bgcolor="#212d42" height="50" width="50">&nbsp;&nbsp;</td>
						<td width="867"><strong>&nbsp;&nbsp;&nbsp; usted se encuentra en :</strong><span class="txtcontenidoazul">|
						<a href="index_menu.php">inicio</a> |
						<span class="txtcontenidoazul">ubicación</span></span></td>
					</tr>
					<tr>
						<td colspan="2">
						<img height="3" src="imagenes/blank.gif" width="1" ></td>
					</tr>
					...
					<tr>
						...
						<table align="center" border="0" cellpadding="0" cellspacing="0" width="98%">
							<tr>
								<td background="imagenes/fill_tablelogin.gif" height="70">
								<table align="center" border="0" cellpadding="0" cellspacing="0" width="98%">
									<tr>
										<td class="titulo3"><h1>&nbsp;</h1></td>
										<td>
										</td>
										<td class="titulo3"></td>
										<td>
							...
							<tr>
								<td class="txtcontenido" valign="top">
									</td>
							</tr>
						</table>
                        </td>
...
$mon = mysql_query("select monto, clave, nombre from departamento where id_departamento=".$clvdepa);
...
?>
  ...
  <table width="80%" border="0" align="center">
    ...
    <tr>
     ...
     <tr>
      <td align="center">Departamento:<?=htmlentities($grantotal[2]);?> </td>
    </tr>
  </table>
  <p>&nbsp;</p>
  ...
  <table width="<?=$totabla;?>" border="0" align="center">
    <tr>
      ...
      <td width="190"><input name="clvdeparta" type="hidden" id="clvdeparta" value="<?=$grantotal[1];?>"/>
        Nombre</td>
	  <?
	  $a=1;
	  while($row = mysql_fetch_array($cadena_sql)){?>
      <td align="center" width="<?=$wit;?>">
      <input name="hidmeta_<?=$a;?>" type="hidden" id="hidmeta" value="<?=$row[0];?>"/>
	  <input name="hidmeta_<?=$a;?>" type="text" id="hidmeta" value="<?=$row[0];?>" size="8" readonly="true"/></td>
	  <? 
	  $a++;
	  }?>
...
<?
	$diez=$w;
	$diez=$diez."0";
	$cad = mysql_query("select clave, nombre, id_partida from partida where estado=1 and clave BETWEEN ".$diez." and ".($diez+9999)." order by clave"); 
	$coco=mysql_num_rows($cad);
?>	  
    <tr>
      <td bgcolor="#CCCCCC">
		<table width="<?=$totabla;?>"  border="0" cellpadding="0" cellspacing="0">
		<tr align="center" onClick="muestra_dtos(<?=$w;?>);">
		<td width="250">	  
	  <strong>
        <input name="tot_<?=$w;?>_partidas" type="hidden" id="tot_<?=$w;?>_partidas" value="<?=$coco;?>"/>
      <label style="cursor:pointer">Capitulo <?=$w;?></label> </strong></td>
 <? for($i=1;$i<=$con;$i++){?>	  
...
?>
      <td bgcolor="#CCCCCC" width="80"><input name="txtall_<?=$w;?>" type="text" id="txtall_<?=$w;?>" size="9" readonly="readonly"/>
	...
	while($mil = mysql_fetch_array($cad)){?>		
			<tr onmouseover="this.style.background='#E9AD72';" onmouseout="this.style.background='#FFFFFF';">
      			<td width="60"><?=$mil[0];?></td>
      			<td width="190" style="font-size:14px"><?=strtolower(htmlentities($mil[1]));?></td>
				<?
 ...
 while($milpa = mysql_fetch_array($palmil)){
	...
	$dinero=mysql_fetch_array(mysql_query("select ingreso_propio from partida_departamento where id_meta=".$milpa[0]." and id_partida=".$mil[2]." and id_departamento=".$clvdepa));

?>  
   			  <td align="center" width="<?=$wit;?>">
                   <input name="txt_<?=$w;?>_<?=$conmil;?>_hidmeta_<?=$i;?>" type="text" id="txt_<?=$w;?>_<?=$conmil;?>_hidmeta_<?=$i;?>" onblur="cal(<?=$conmil;?>,<?=$i;?>,<?=$w;?>);" onkeypress="return decimales(event);" size="8" value="<? if($dinero[0]!=NULL){ echo "$".number_format($dinero[0]);}?>" title="<?=$mil[0];?>"/>
...
if(isset($_POST['TOTAL']) && $_POST['TOTAL']!='')
{
	...
	for($c=1000;$c<=9000;$c+=1000)
	{
		...
		$diez=$c;
	$diez=$diez."0";
		$aracapi=mysql_fetch_array(mysql_query("select SUM(p.ingreso_propio) from partida_departamento p join partida pa on p.id_partida=pa.id_partida where pa.clave BETWEEN ".$diez." and (".$diez." +9999)"));
		if($aracapi[0]!=NULL)
		{
			$money=$aracapi[0];
		}else{
	...
	if(!($cad==''))
	{
		$cad.="&sum=".$sum;
		$cad.="&ideparta=".$depar;	
		echo '<script lenguaje="javascript">open("cincomil2.php?'.$cad.'","_self");</script>';		
	}		 
