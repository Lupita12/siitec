...
var TableGear = new Class({
  ...
  nextRequest: function(){
    ...
    params[name.full] = input.get("value");
    if(name.key) params["edit[]"] = name.key;
    else params["insert"] = true;
  ...
  onComplete: function(response){
    ...
    if(!json || json.affected <= 0){
      ...
      if(this.currentJob.input.get("name") == json.name){
      ...
      if(json.action == "insert" && json.key){
