...
function enviar(campo)
{
	with(document.form1)
	{
		...
		while(variable != campo)
		{
			...
			if(document.getElementById('txtfechainicial_'+variable).value==''){
				alert("INGRESA UNA FECHA INICIAL");document.getElementById('txtfechainicial_'+variable).focus();return;
				vari++;
				document.write(vari);
			}
			if(document.getElementById('txtfechafinal_'+variable).value==''){
				alert("INGRESA UNA FECHA INICIAL");document.getElementById('txtfechafinal_'+variable).focus();return;
				vari++;
				document.write(vari);
			}
			variable++;
...
<table width="98%" border="0">
 <tr>
  <th width="19%" height="24">Seleccione Gasto Propio:</th>
   <th width="20%">
...
if(isset($_POST['combo']) && $_POST['combo']!='')
{
	...
	$Sql="SELECT * FROM gastodirecto WHERE id_gastodirecto = '$clavesesion'";
	$ISql = mysql_query($Sql);
	$num=mysql_num_rows($ISql);	
?>
<table width="98%" border="0" align="center">
    <tr>
      <th width="166" align="left">Clave</th>
      <th width="173" align="left">Nombre</th>
      <th width="345" align="left">Monto</th>
      <th width="345" align="left">Fecha Inicial</th>
      <th width="345" align="left">Fecha Final</th>
    </tr>
<?PHP
	if($num>0)
	{
		...
		{?>
			...
			<? $a=0;}?>
		      	...
		      	<td><? echo $row[1]; ?></td>
      			<td><p><input name="txtmonto_<?=$con;?>" type="text" onkeypress="return decimales(event,this);"  id="txtmonto_<?=$con;?>" size="14" value="<?=$row[4];?>" /></p></td>
      			<td><p><label for="txtfechainicial_<?=$con;?>"><input name="txtfechainicial_<?=$con;?>" type="text" class="fecha post" id="txtfechainicial_<?=$con;?>" size="12" readonly="readonly" value="<?=$row[5];?>" /></label></p></td>
      			<td><p><label for="txtfechafinal_<?=$con;?>"><input name="txtfechafinal_<?=$con;?>" type="text" class="fecha post" id="txtfechafinal_<?=$con;?>" size="12" readonly="readonly" value="<?=$row[6];?>" /></label></p></td>
   			</tr>
...
if(isset($_POST['txtmonto_1']) && $_POST['txtmonto_1']!='')
{
		...
		for($a=1;$a<=$cont;$a++)
		{
			...
			$fecha_inicial=$_POST["txtfechainicial_".$a];
			$fecha_final=$_POST["txtfechafinal_".$a];			
			$vigencia=date('Y/m/d');
			$GastoDirecto = new GastoDirecto($clave);
			$GastoDirecto->setmonto($monto);
			$GastoDirecto->setvigencia($vigencia);
...
<link type="text/css" rel="stylesheet" href="stylesheets/TableGear.css" />
						...
						<span class="txtcontenidoazul">Distribución Gasto Directo</span><!-- InstanceEndEditable -->
</body>
